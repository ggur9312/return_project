...
- `dashboard.js`'s `dashboardRows` (`{createdAt, status, qtyTotal, qtyPicked, qtyLoaded}`, persisted under `pickListDashboardData`) is deliberately **not** on the shared `state` object — like `extract.js`'s `extractedRows`, it's a module-local var in the owning file, since no other `js/pick/*.js` file needs to read it.

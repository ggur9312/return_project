...
js/pick/dashboard.js    -- 집품 현황: 홈 대시보드 screen (독립 A~N 엑셀 업로드 + Chart.js KPI/도넛/막대 그래프)
...
| `dashboard.js` | 290 | 홈 대시보드(`#dashboardView`, nav의 "홈" — 기본 진입 화면): 자체 독립 데이터셋(`dashboardRows`, `state.rows`와 완전히 별개, localStorage 키 `pickListDashboardData`) — A~N(14열) 원본 엑셀/붙여넣기를 포지셔널로 파싱(C=내부반출번호, D=생성일시, N=상태값, J/K/L=수량 3단계), 4개 KPI 카드 + Chart.js 도넛 2개(집품/상차 현황, 남은 수량 계산) 렌더링. 업로드는 교체가 아니라 **병합** — C열(내부반출번호)을 dedup key로 이미 존재하는 행은 건너뛴다(`mergeDashboardRows`). 날짜 탭에 `.date-tab-close`로 날짜별 삭제(`removeDashboardRowsByDate`, core.js의 `removeRowsByDate`와 동일 패턴) 가능. 하단 존별 막대그래프만 예외적으로 `state.rows`(집품리스트 현황 데이터)를 사용 — 서로 다른 데이터셋을 한 화면에서 같이 보여주는 유일한 지점이니 헷갈리지 말 것. 날짜 탭(`dashboardActiveDateTabs`)은 `renderDateTabs`와 별개의 자체 구현, `dashboardRows`의 생성일시만 기준으로 삼음(존 막대그래프는 날짜 탭 필터에 영향받지 않음, 항상 전체 `state.rows` 기준) |
